<?php 
 ...
 if (isset($_POST['addDevice'])) {
    ...
    if (empty($_name) && empty($maker) && empty($model) && empty($year) && empty($stock) && empty($loans)) {
        $_SESSION['nofound'] =  "<div class='nofound'>Et voi lisätä tyhjiä syöttöjä</div>";
    } else {
        $sql = "INSERT INTO laite (device, maker, model, year, stock, loans) VALUES ( '$device', '$maker', '$model', '$year', '$stock', '$loans' )";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            $_SESSION['addedbook'] = "<div class='addedbook'>Laite lisää onnistui</div>";
 ...
 if (isset($_POST['searchdevice'])) {
    ...
    if ($search == "") {
        ...
        $sql_search = "SELECT * FROM laite WHERE id LIKE '%$search%' or name LIKE '%$search%'";
        $result_search = mysqli_query($conn, $sql_search);
        if ($result_search) {
            $num_search = mysqli_num_rows($result_search);
            if ($num_search > 0) {
            while ($row = mysqli_fetch_assoc($result_search)) {
                $_SESSION['search'] = "<table class='table laite'><tbody>
                    <tr>
                    <th>$row[id]</th>
                    <td>$row[device]</td>
                    <td>$row[maker]</td>
...
.dropdown-menu li:hover {
   ...
   .svg-a {
    ...
    body {
        ...
        background: url("https://img.pikbest.com/ai/illus_our/20230526/bce91e721f2f2d2a021ae0236f90fc4e.jpg!bw700");
...
body::-webkit-scrollbar {
 ...
 .input-search {
    ...
    .poup-container {
        ...
        margin: auto;
        padding: 30px 10px;
        background-image: url(" https://th.bing.com/th/id/OIP.rAD8xTOS5R-XLC9q1Fa1PwHaEK?w=296&h=180&c=7&r=0&o=5&dpr=1.5&pid=1.7");
        background-repeat: no-repeat;
        background-position: center;
        background-size: cover;
        border-radius: 4px;
...
<header>
    <nav>
        <div style="margin-top: 5px;">
          ...
          <ul>
                ...
                <li><a href="device.php">Laite</a></li>
  ...
  <div class="search-input">
            <form action="" method="post">
                ...
                <button type="submit" class="hae" name="searchdevice">Etsi laite</button>
            </form>
         </div>
    <div>
...
<div style="margin: 54px;">
    <button type="button" class="btn btn-danger" id="pop-btn-open">Lisää Laite</button>
  ...
  <div>
    <div class="pop">
      <div style="display: flex; justify-content: space-between;">
        <h5>Lisää Kirja</h5>
        <div id="closePopup">
        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16"><path fill="currentColor" d="m16 0l-1 .01L8 7L1 .01L0 0v1l7 7l-7 7v1h1l7-7l7 7h1v-1L9 8l7-7z"/></svg>
        </div>
      </div>
      <hr>
      <div class="modal-body">
        <form action="devices.php" method="post">
          <div class="mb-3">
            <label for="device" class="form-label">Laite Nimi</label>
            <input type="text" class="form-control" name="device" maxlength="12" placeholder="laite" require>
          </div>
          <div class="mb-3">
            <label for="maker" class="form-label">Laite luoja</label>
            <input type="text" class="form-control" name="maker" maxlength="4" placeholder="laite luoja">
          </div>
          <div class="mb-3">
            <label for="model" class="form-label">Laite malli</label>
            <input type="text" class="form-control" name="model" maxlength="12"placeholder="Laite malli">
          </div>
          <div class="mb-3">
            <label for="year" class="form-label">Laite Vuosi</label>
            <input type="text" class="form-control" name="year" maxlength="4" placeholder="Laite vuosi">
          </div>
          <div class="mb-3">
            <label for="stock" class="form-label">Laite varastossa</label>
            <input type="text" class="form-control" name="stock" maxlength="20" placeholder="varastossa">
          </div>
         <div class="mb-3">
            <label for="loans" class="form-label">Laite lainat</label>
            <input type="text" class="form-control" name="loans" maxlength="20" placeholder="lainat">
          </div>
          <hr>
          <div >
            <button type="button" id="close-btn" class="btn btn-dark" data-bs-dismiss="modal">Sulja</button>
            <button type="submit" id="adddBook" class="btn btn-danger" name="addDevice">Lisää Kirja</button>
          </div>
        </form>
      </div>
    </div>
...
<!-- end  pop for add book -->
    ...
    <div class="container-display">
        ...
        ?>
        <table class="table laite mt-5" id="table">
            <thead>
                <tr>
                <th scope="col">Laite Nimi</th>
                <th scope="col">Laite luoja</th>
                <th scope="col">Laite malli</th>
                <th scope="col">Laite vuosi</th>
                <th scope="col">Laite varastossa</th>
                <th scope="col">Laite lainat</th>
               ...
               while($row = mysqli_fetch_assoc($result)) {
                $id = $row['id'];
                  echo "
                        ...
                        <td style='text-align: center; align-items: center;'>
                            <button class='btn btn-danger border border-ligh'>
                                <a href='deleteDevice.php?id=$row[id]'>Poista</a>
                                <svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em' viewBox='0 0 2048 2048'><path fill='currentColor' d='M1792 384h-128v1472q0 40-15 75t-41 61t-61 41t-75 15H448q-40 0-75-15t-61-41t-41-61t-15-75V384H128V256h512V128q0-27 10-50t27-40t41-28t50-10h384q27 0 50 10t40 27t28 41t10 50v128h512zM768 256h384V128H768zm768 128H384v1472q0 26 19 45t45 19h1024q26 0 45-19t19-45zM768 1664H640V640h128zm256 0H896V640h128zm256 0h-128V640h128z'/></svg>
                            </button>
                        </td>
                        </tr>
