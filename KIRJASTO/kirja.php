...
if (isset($_POST['addBook'])) {
    ...
    if (empty($name) && empty($year) && empty($author) && empty($publick) && empty($library) && empty($isbn)) {
        $_SESSION['nofound'] =  "<div class='nofound'>Et voi lisätä tyhjiä syöttöjä</div>";
    } else {
        $sql = "INSERT INTO kirja (name, year, author, publick, library, isbn) VALUES ( '$name', '$year', '$author', '$publick', '$library', '$isbn' )";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            $_SESSION['addedbook'] = "<div class='addedbook'>Kirja lisää onnistui</div>";
...
if (isset($_POST['searchbook'])) {
    ...
    if ($search == "") {
        ...
        $sql_search = "SELECT * FROM kirja WHERE id LIKE '%$search%' or name LIKE '%$search%'";
...
.dropdown-menu li:hover {
    ...
    body {
        ...
        background: url("https://img.freepik.com/premium-photo/atmosphere-library-reading-book-generative-ai_669646-628.jpg");
    ...
    .poup-container {
        ...
        margin: auto;
        padding: 30px 10px;
        background: url("https://img.freepik.com/premium-photo/library-dark_935641-162.jpg");
        background-repeat: no-repeat;
        background-position: center;
    ...
    .kirja {
        background: url("https://c0.wallpaperflare.com/preview/125/782/479/black-and-white-wooden-shelf.jpg");
    }
</style>
<body>
        ...
        <nav>
           ...
           <div class="search-input">
            <form action="" method="post">
                <input type="text" name="search" placeholder="Etsi nimi...">
                <svg class="icon" name="submit" xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24">
                    <path fill="none" stroke="Aqua" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10a7 7 0 1 0 14 0a7 7 0 1 0-14 0m18 11l-6-6" />
                </svg>
                <button type="submit" class="hae" name="searchbook">Etsi Kirja</button>
            </form>
        </div>
        <div>
            <svg id="menu" xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24">
                <path fill="none" stroke="Aqua" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12h16M4 6h16M4 18h16" />
            </svg>
        </div>
    </header>
    ...
    <section>
        <div>
            <div class="admin">
                <div>
                    <svg id="close" xmlns="http://www.w3.org/2000/svg" width="30px" height="30px" viewBox="0 0 24 24">
                        <path fill="none" stroke="Aqua" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m7 7l10 10M7 17L17 7" />
                    </svg>
                </div>
                <div class="admin-a">
                    <div class="svg-a">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24">
                            <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2">
                                <path stroke-dasharray="32" stroke-dashoffset="32" d="M13 4L20 4C20.5523 4 21 4.44772 21 5V19C21 19.5523 20.5523 20 20 20H13">
                                ...
                                <path stroke-dasharray="6" stroke-dashoffset="6" d="M14.5 12l-3.5 -3.5M14.5 12l-3.5 3.5" opacity="0">
    ...
    <div style="margin: 54px;">
        <button type="button" class="btn btn-danger" id="pop-btn-open">Lisää Kirjaa</button>
        ...
        <div>
            <div class="pop">
                ...
                <div class="modal-body">
                    <form action="kirja.php" method="post">
                        <div class="mb-3">
                            <label for="name" class="form-label">Kirja Nimi</label>
                            <input type="text" class="form-control" name="name" maxlength="12" placeholder="Kirja nimi">
                        </div>
                        <div class="mb-3">
                            <label for="year" class="form-label">Kirja Vuosi</label>
                            <input type="text" class="form-control" name="year" maxlength="4" placeholder="kirja vuosi">
                        </div>
                        <div class="mb-3">
                            <label for="author" class="form-label">Kirjailija Nimi</label>
                            <input type="text" class="form-control" name="author" maxlength="12" placeholder="Kirjailija nimi">
                        </div>
                        <div class="mb-3">
                            <label for="publick" class="form-label">Julkaisija Vuosi</label>
                            <input type="text" class="form-control" name="publick" maxlength="4" placeholder="Jlkaisija vuosi">
                        </div>
                        <div class="mb-3">
                            <label for="library" class="form-label">Kirjasto Nimi</label>
                            <input type="text" class="form-control" name="library" maxlength="20" placeholder="Kirjasto nimi">
                        </div>
                        <div class="mb-3">
                            <label for="isbn" class="form-label">ISBN</label>
                            <input type="text" class="form-control" name="isbn" maxlength="13" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="1234567890">
                        </div>
                        <hr>

                        <div>
                            <button type="button" id="close-btn" class="btn btn-dark" data-bs-dismiss="modal">Sulja</button>
                            <button type="submit" id="adddBook" class="btn btn-danger" name="addBook">Lisää Kirja</button>
                        </div>
                    </form>
                </div>
...
<!-- end  pop for add book -->


<div class="container-display">

<?php
  if (isset($_SESSION['addedbook'])) {
  echo $_SESSION['addedbook'];
...
?>
<table class="table kirja mt-5" id="table">
<thead>
 <tr>
   <th scope="col">Kirja Nimi</th>
   <th scope="col">Kirjan Vuosi</th>
   <th scope="col">Kirjailija Nimi</th>
   <th scope="col">Julkaisija Vuosi</th>
   <th scope="col">Kirjasto Nimi</th>
 ...
 while ($row = mysqli_fetch_assoc($result)) {
  $id = $row['id'];
                   echo "
                        ...
                        <td style='text-align: center; align-items: center;'>
                          <button class='btn btn-danger border border-ligh'>
                          <a href='deleteBook.php?id=$row[id];'>Poista</a>
                          <svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em'viewBox='0 0 2048 2048'><path fill='currentColor' d='M1792 384h-128v1472q0 40-15 75t-41 61t-61 41t-75 15H448q-40 0-75-15t-61-41t-41-61t-15-75V384H128V256h512V128q0-27 10-50t27-40t41-28t50-10h384q27 0 50 10t40 27t28 41t10 50v128h512zM768 256h384V128H768zm768 128H384v1472q0 26 19 45t45 19h1024q26 0 45-19t19-45zM768 1664H640V640h128zm256 0H896V640h128zm256 0h-128V640h128z'/></svg>
                        </button>
                      </td>
                   </tr>
    ...
    <!-- javascript file -->
    <script>
        const popup = document.querySelector(".poup-container");
        const openPopup = document.getElementById("pop-btn-open");
        const table = document.getElementById("table");
        const closePopup = document.getElementById("closePopup");
        const closeButton = document.getElementById("close-btn");
        const adddBook = document.getElementById("adddBook");
        /* header */
        ...
        adddBook.addEventListener("click", () => {
            popup.style.display = "none";
            table.style.display = "inline-table";
            openPopup.style.display = "block";
