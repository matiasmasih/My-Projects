...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($firstname)) {
        ...
        try {
            ...
            $stmt->execute([$email]);
            if ($stmt->rowCount() > 0) {
                $_SESSION['email-exist'] = "<div class='email-exist'>Sähköpostiosoite on jo olemassa, kokeile toista!</div>";
            } else if ($password !== $cpassword) {
                $_SESSION['password-not-match'] = "<div class='password-not-match'>Salasana ja vahvista salasana eivät täsmää</div>";
            } else {
                // Insert new user
                $stmt = $pdo->prepare("INSERT INTO register (firstname, lastname, email, password, cpassword) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$firstname, $lastname, $email, $password, $cpassword]);
