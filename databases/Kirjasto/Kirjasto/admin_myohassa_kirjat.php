<?php
// admin_myohassa_kirjat.php - Myöhässä olevat kirjat
session_start();
...
// GET parameters for search/filter
...
// Build WHERE conditions for overdue loans
$where_conditions = ["l.erapaiva < ?", "l.tila = 'aktiivinen'"];
$params = [$today];
$types = 's';
...
$lainat_sql = "SELECT
    ...
    (DATEDIFF(CURDATE(), l.erapaiva) * 0.50) as ehdotettu_sakko
    FROM lainat l
    JOIN kirjat k ON l.kirja_id = k.id
    JOIN jasenet j ON l.jasen_id = j.id
    $where_sql
    ORDER BY l.erapaiva ASC, l.id DESC
    LIMIT ? OFFSET ?";
...
// Get statistics for overdue books
$stats_sql = "SELECT
    COUNT(*) as total_myohassa,
    COALESCE(SUM(DATEDIFF(CURDATE(), erapaiva)), 0) as yhteensa_paivia_myohassa,
    COALESCE(SUM(sakot), 0) as total_sakot,
    COALESCE(SUM(DATEDIFF(CURDATE(), erapaiva) * 0.50), 0) as mahdolliset_sakot
    FROM lainat
    WHERE erapaiva < CURDATE() AND tila = 'aktiivinen'";

$stats_result = $conn->query($stats_sql);
$stats = $stats_result ? $stats_result->fetch_assoc() : [
    'total_myohassa' => 0,
    'yhteensa_paivia_myohassa' => 0,
    'total_sakot' => 0,
    'mahdolliset_sakot' => 0
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['mark_returned'])) {
        ...
        if ($laina_id > 0) {
            $update_sql = "UPDATE lainat
                          SET tila = 'palautettu',
                              palautuspaiva = CURDATE(),
                              sakot = ?
                          WHERE id = ?";
            ...
            if ($update_stmt) {
                ...
                if ($update_stmt->execute()) {
                    ...
                    header("Location: admin_myohassa_kirjat.php?success=1");
                    exit();
                } else {
                    $errors[] = "Lainan päivitys epäonnistui";
                }
            }
        }
    }
        ...
        if (empty($selected_lainat)) {
            ...
            foreach ($selected_lainat as $laina_id) {
                ...
                if ($laina_id > 0 && $action == 'return') {
                    $fine_sql = "SELECT DATEDIFF(CURDATE(), erapaiva) * 0.50 as suggested_fine FROM lainat WHERE id = ?";
                    $fine_stmt = $conn->prepare($fine_sql);
                    $fine_stmt->bind_param("i", $laina_id);
                    $fine_stmt->execute();
                    $fine_result = $fine_stmt->get_result();
                    $fine_data = $fine_result->fetch_assoc();
                    $suggested_fine = $fine_data['suggested_fine'] ?? 0;
                    ...
                    $update_sql = "UPDATE lainat
                                  ...
                                  WHERE id = ?";
                    $update_stmt = $conn->prepare($update_sql);
                    $update_stmt->bind_param("di", $suggested_fine, $laina_id);
                    if ($update_stmt->execute()) {
                        $updated_count++;
                    }
                }
            ...
            if ($updated_count > 0) {
                $success[] = "$updated_count lainaa käsitelty onnistuneesti!";
                header("Location: admin_myohassa_kirjat.php?success=1");
                exit();
            }
        }
...
<head>
    ...
    <title>Myöhässä Olevat Kirjat | Admin | Kirjasto</title>
...
<div class="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <a href="admin_myohassa_kirjat.php" class="menu-item active"><i class="fas fa-clock"></i><span>Myöhässä Olevat</span></a>
...
<div class="main-content">
    ...
    <div class="header">
        <div class="page-title">
            <h1><i class="fas fa-clock"></i> Myöhässä Olevat Kirjat</h1>
            <p><i class="fas fa-exclamation-triangle"></i> Hallinnoi myöhässä olevia lainoja ja sakkoja</p>
        </div>
        <div class="user-info">
    ...
    <div class="warning-alert">
        ...
        <div class="warning-alert-content">
            ...
            <p>Myöhässä olevista kirjoista peritään automaattisesti 0,50 €/päivä sakkoa. Voit muokata sakon määrää palautettaessa kirjaa.</p>
        </div>
    </div>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-header">
                <div class="stat-info">
                    <h3>Myöhässä olevat</h3>
                    <div class="stat-number"><?php echo number_format($stats['total_myohassa'], 0, ',', ' '); ?></div>
                </div>
        ...
        <div class="stat-card">
            <div class="stat-header">
                <div class="stat-info">
                    <h3>Yhteensä päiviä</h3>
                    <div class="stat-number"><?php echo number_format($stats['yhteensa_paivia_myohassa'], 0, ',', ' '); ?></div>
                </div>
        ...
        <div class="stat-card">
            <div class="stat-header">
                <div class="stat-info">
                    <h3>Mahdolliset sakot</h3>
                    <div class="stat-number"><?php echo number_format($stats['mahdolliset_sakot'], 2, ',', ' '); ?>€</div>
                </div>
    ...
    <div class="filter-section">
        <div class="section-title">
            <i class="fas fa-search"></i> Hae myöhässä olevia lainoja
        </div>
            ...
            <div class="search-filter">
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <a href="admin_myohassa_kirjat.php" class="btn btn-light"><i class="fas fa-times"></i> Tyhjennä</a>
                    </div>
                </div>
            </div>
        </form>
    ...
    <div class="returns-section">
        <div class="section-title">
            <i class="fas fa-list"></i> Myöhässä olevat lainat (<?php echo $total_lainat; ?> kpl)
        </div>
        ...
        <form method="POST" action="" id="bulkForm">
            <div class="bulk-actions">
                <div style="display: flex; align-items: center; gap: 12px;">
                    ...
                    <select name="bulk_action" class="form-control" style="width: auto; min-width: 150px;" onchange="if(this.value && confirm('Haluatko varmasti merkitä valitut lainat palautetuksi?')) this.form.submit()">
                        <option value="">Valitse toiminto...</option>
                        <option value="return">Palauta kaikki (ehdotettu sakko)</option>
                    </select>
                </div>
            </div>
            ...
            <?php if (empty($lainat)): ?>
                <div class="empty-state">
                    <i class="fas fa-check-circle"></i>
                    <h3>Ei myöhässä olevia lainoja!</h3>
                    <p>Kaikki kirjat ovat ajallaan palautettu. Hienoa työtä! 🎉</p>
                </div>
            <?php else: ?>
                    ...
                    <table class="overdue-table">
                        <thead>
                            <tr>
                                ...
                                <th>Eräpäivä</th>
                                <th>Myöhässä</th>
                                <th>Nykyinen sakko</th>
                                <th>Ehdotettu sakko</th>
                                <th>Toiminnot</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="date-overdue"><?php echo date('d.m.Y', strtotime($laina['erapaiva'])); ?></td>
                                <td>
                                    <span class="status-badge status-overdue">
                                        <i class="fas fa-clock"></i> <?php echo $laina['paivia_myohassa']; ?> pv
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class="fine-amount"><?php echo number_format($laina['ehdotettu_sakko'], 2, ',', ' '); ?> €</span>
                                </td>
                                <td>
                                    <div class="table-actions">
                                        <button type="button" class="btn btn-success btn-sm" onclick="openReturnModal(<?php echo $laina['laina_id']; ?>, '<?php echo addslashes($laina['kirja_nimi']); ?>', <?php echo $laina['ehdotettu_sakko']; ?>)">
                                            <i class="fas fa-check"></i> Palauta
                                        </button>
                                        <button type="button" class="btn btn-warning btn-sm" onclick="sendReminder('<?php echo addslashes($laina['jasen_email']); ?>')">
                                            <i class="fas fa-envelope"></i> Muistuta
                                        </button>
                                    </div>
                                </td>
                            </tr>
    ...
    <div class="modal-content">
        ...
        <form method="POST" action="" id="returnForm">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <div style="font-size: 0.7rem; color: #64748b; margin-top: 5px;">Ehdotettu sakko: <span id="suggestedFine">0.00</span> € (0,50 €/päivä)</div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.stat-card, .filter-section, .returns-section, .warning-alert').forEach(function(el) {
            el.style.opacity = '0';
    ...
    function openReturnModal(lainaId, bookTitle, suggestedFine) {
        document.getElementById('returnLainaId').value = lainaId;
        document.getElementById('sakko').value = suggestedFine.toFixed(2);
        document.getElementById('suggestedFine').textContent = suggestedFine.toFixed(2);
    ...
    function sendReminder(email) {
        if (confirm('Haluatko lähettää muistutuksen sähköpostilla osoitteeseen: ' + email + '?')) {
            alert('Muistutus lähetetty osoitteeseen: ' + email);
        }
    }
